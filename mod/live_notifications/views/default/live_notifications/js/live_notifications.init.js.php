...
$(document).ready(function () {
    ...
    if(elgg.is_logged_in()){
       ...
       setInterval(function() {
        ...
        <?php if (elgg_is_active_plugin('time_theme') || elgg_is_active_plugin('time_theme_pro')): ?>
        //check new messages
        setInterval(function() {
            ...
            elgg.action('live_notifications/refresh_count_messages', function(data) {
                ...
                var text = '<i class="fa fa-envelope fa-lg"></i>';

                // get unread messages
                
                var num_messages = parseInt(data.output);
    
                var num = parseInt($(".elgg-menu-item-messages a span.elgg-topbar-new").html());
                if(num_messages>num || num_messages > 0){
                    text = text + "<span class=\"elgg-topbar-new\">"+num_messages+"</span>";
                    $(".elgg-menu-item-messages a").html(text);
                    //$(".messages-new").show();
                }
                else if(num_messages==0){
                    $(".elgg-menu-item-messages a span.elgg-topbar-new").hide();               
                }
            });
